...
import lattice, detect, route, translate

type
  InferenceFn* = proc(prompt: string, system_prompt: string): Result[string, BridgeError] {.raises: [].}
...
proc query*(s: var LinguaLlmSession, text: string): Result[string, BridgeError] =
  inc s.queries
  let lang = detect(text, s.detect_fn)
  if lang.is_bad: return Result[string, BridgeError].bad(lang.err)
  let routed = route(lang.val, s.route_config)
  var prompt = text
  if s.use_translation:
    let translated = pre_translate(s.translate_layer, text, lang.val)
    if translated.is_bad: return Result[string, BridgeError].bad(translated.err)
